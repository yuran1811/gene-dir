...
(async () => {
  ...
  const templateId = Object.entries(templates).find(([key, val]) => genFromTp == key || val.desc === genFromTp)[0];
  ...
  for (const key in genOpts)
    (key == genOpt || genOpts[key].desc === genOpt) && genOpts[key].setup(directory, name, noFolderIn);
  ...
  if (!!genUseTp) {
    ...
    if (!_fs.dir.exist(tpDir)) _fs.f.write(tpDir, cppDefault(genUseTp, noFolderIn));
