...
const geneOptions = {
  f: {
    ...
    setup: (directory, name, aio = 0) => {
      ...
      for (let i = 0; i <= 3; i++) _fs.f.write(dirPath(inDir, `in${i ? i : ''}`), defaultMes);
      ...
      // test dir
      _fs.f.write(dirPath(testDir, `brute.cpp`), cppDefault('brute', aio, { type: 'brute' }));
      _fs.f.write(dirPath(testDir, `gen.cpp`), cppDefault('gen', aio, { type: 'gen' }));
    },
  },
  io: {
    name: 'io',
